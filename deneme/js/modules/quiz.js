...
function renderQuizQ() {
    ...
    let opts = words.filter(w => w.en !== target.en)
...
function finishQuiz() {
    ...
    if (store.state.activeItems.doubleXP > 0) {
        finalXP = baseXP * 2;
        store.update('activeItems', { 
            ...store.state.activeItems, 
            doubleXP: Math.max(0, store.state.activeItems.doubleXP - 20) 
        });
    }

    store.update('xp', store.state.xp + finalXP);
    store.updateHistory(finalXP);
